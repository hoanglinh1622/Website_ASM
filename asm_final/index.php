...
<head>
    ...
    <style>
       body { font-family: 'Arial', sans-serif; margin: 0; background: #f8f9fa; color: #333; }
        ...
        .pagination a { padding: 10px; margin: 5px; border-radius: 5px; text-decoration: none; background: #007bff; color: white; display: inline-block; }
        ...
        input[type="text"] {width: 100%;max-width: 500px; padding: 12px 15px 12px 40px; border: 1px solid #ddd; border-radius: 25px; 
                            box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1); background: white; transition: all 0.3s;}
        input[type="text"]:focus { border-color: #007bff; outline: none; box-shadow: 0px 4px 15px rgba(0, 123, 255, 0.3); }

        button {position: absolute; left:300px ; top: 50%; transform: translateY(-50%); background:rgb(38, 180, 19); color: white; border: none; border-radius: 50px; padding: 8px 15px;
                 cursor: pointer;font-size: 16px;box-shadow: 0px 4px 10px rgba(0, 0, 0, 0.1); transition: background 0.3s; }
        button:hover {background: #0056b3; }
        .search-box::before {position: absolute; left: 15px; top: 50%; transform: translateY(-50%); color: #999; font-size: 20px; }
    </style>
</head>
<body>
    <div class="header">
        <a href="index.php"><i class="fa fa-home"></i> Sales Page</a>
    ...
    <div class="container">
        <div class="sidebar">
            ...
            <?php foreach ($categories as $cat): ?>
                <p><a href="?category=<?php echo $cat['id']; ?>">👉 <?php echo $cat['name']; ?></a></p>
            <?php endforeach; ?>
            <h3>🏷️ Brand</h3>
            <?php foreach ($brands as $brand): ?>
                <p><a href="?brand=<?php echo $brand['id']; ?>">🔹 <?php echo $brand['name']; ?></a></p>
            <?php endforeach; ?>
        </div>
